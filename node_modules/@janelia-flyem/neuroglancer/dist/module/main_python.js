...
import { VIEWER_UI_CONFIG_OPTIONS } from './neuroglancer/viewer';
function makeTrackableBasedEventActionMaps(inputEventBindings) {
...
function makeTrackableBasedSourceGenerationHandler(pythonDataSource) {
    const state = new TrackableValue({}, x => {
        for (const key of _Object$keys(x)) {
            const value = x[key];
            if (typeof value !== 'number') {
                throw new Error(`Expected key ${_JSON$stringify(key)} to have a numeric value, but received: ${_JSON$stringify(value)}.`);
            }
        }
        return x;
    });
    state.changed.add(debounce(() => {
        ...
        for (const key of _Object$keys(generations)) {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const dataSourceProvider = getDefaultDataSourceProvider({ credentialsManager: new CachingCredentialsManager(credentialsManager) });
    ...
    configState.add('inputEventBindings', makeTrackableBasedEventActionMaps(viewer.inputEventBindings));
    const remoteActionHandler = new RemoteActionHandler(viewer);
    window['remoteActionHandler'] = remoteActionHandler;
    ...
    let sharedState = viewer.state;
    if (window.location.hash) {
        const hashBinding = viewer.registerDisposer(new UrlHashBinding(viewer.state, credentialsManager));
        hashBinding.updateFromUrlHash();
        sharedState = undefined;
    }
    const prefetchManager = new PrefetchManager(viewer.display, dataSourceProvider, viewer.dataContext.addRef(), viewer.uiConfiguration);
    ...
    const size = new TrackableValue(undefined, x => x == null ? undefined : parseFixedLengthArray([0, 0], x, verifyInt));
    ...
    remoteActionHandler.sendActionRequested.add((action, state) => client.sendActionNotification(action, state));
    screenshotHandler.sendScreenshotRequested.add(state => client.sendActionNotification('screenshot', state));
    screenshotHandler.sendStatisticsRequested.add(state => client.sendActionNotification('screenshotStatistics', state));
