...
import { DataType, makeVolumeChunkSpecification, VolumeType } from '../../sliceview/volume/base';
...
const VALID_ENCODINGS = new _Set(['jpg', 'raw16']);
...
const VALID_STACK_STATES = new _Set(['COMPLETE', 'READ_ONLY']);
const PARTIAL_STACK_STATES = new _Set(['LOADING']);
...
function parseStackInfo(obj) {
    ...
    let voxelResolution = verifyObjectProperty(obj, 'currentVersion', parseStackVersionInfo);
    let project = verifyObjectProperty(obj, 'stackId', parseStackProject);
...
function parseStackVersionInfo(stackVersionObj) {
    ...
    let voxelResolution = vec3.create();
    try {
...
class RenderMultiscaleVolumeChunkSource extends MultiscaleVolumeChunkSource {
    constructor(chunkManager, baseUrl, ownerInfo, stack, project, channel, parameters) {
        ...
        if (stack === undefined) {
            const stackNames = _Array$from(projectInfo.stacks.keys());
    ...
    getSources(volumeSourceOptions) {
        ...
        var _stackInfo = this.stackInfo;
        const baseLowerVoxelBound = _stackInfo.lowerVoxelBound,
              baseUpperVoxelBound = _stackInfo.upperVoxelBound;
...
export function getOwnerInfo(chunkManager, hostname, owner) {
    return chunkManager.memoize.getUncounted({ 'type': 'render:getOwnerInfo', hostname, owner }, () => fetchOk(`${hostname}/render-ws/v1/owner/${owner}/stacks`).then(response => response.json()).then(parseOwnerInfo));
}
const pathPattern = /^([^\/?]+)(?:\/([^\/?]+))?(?:\/([^\/?]+))(?:\/([^\/?]*))?(?:\?(.*))?$/;
...
function getVolume(chunkManager, datasourcePath) {
    ...
    return chunkManager.memoize.getUncounted({ type: 'render:MultiscaleVolumeChunkSource', hostname, path }, async () => {
        const ownerInfo = await getOwnerInfo(chunkManager, hostname, owner);
        const volume = new RenderMultiscaleVolumeChunkSource(chunkManager, hostname, ownerInfo, stack, project, channel, parameters);
        ...
        const dataSource = {
            ...
            subsources: [{
                ...
                id: 'bounds',
                default: true,
                subsource: { staticAnnotations: makeDataBoundsBoundingBoxAnnotationSet(modelSpace.bounds) }
            }]
        };
        return dataSource;
    });
}
export async function stackAndProjectCompleter(chunkManager, hostname, path) {
    ...
    if (stackMatch[3] === undefined) {
        ...
        let completions = getPrefixMatchesWithDescriptions(projectPrefix, ownerInfo.projects, x => x[0] + '/', () => undefined);
    ...
    if (stackMatch[4] === undefined) {
        ...
        let completions = getPrefixMatchesWithDescriptions(stackPrefix, projectInfo.stacks, x => x[0] + '/', x => {
            return x[1].project;
        });
    ...
    let completions = getPrefixMatchesWithDescriptions(channelPrefix, channels, x => x, () => undefined);
    return {
        offset: stackMatch[1].length + stackMatch[2].length + stackMatch[3].length + 3,
        completions
    };
}
export async function volumeCompleter(url, chunkManager) {
