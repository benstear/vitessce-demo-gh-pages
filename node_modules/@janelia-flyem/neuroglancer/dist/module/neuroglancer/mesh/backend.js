...
import { PerspectiveViewRenderLayerBackend } from "../perspective_view/backend";
...
export function decodeVertexPositionsAndIndices(verticesPerPrimitive, data, endianness, vertexByteOffset, numVertices, indexByteOffset, numPrimitives) {
    ...
    let indices = numIndices === undefined ? new Uint32Array(data, indexByteOffset) : new Uint32Array(data, indexByteOffset, numIndices);
...
export class MeshSource extends ChunkSource {
    constructor(rpc, options) {
        super(rpc, options);
        let fragmentSource = this.fragmentSource = this.registerDisposer(rpc.getRef(options['fragmentSource']));
    ...
    getFragmentChunk(manifestChunk, fragmentId) {
        ...
        var _getFragmentKey = this.getFragmentKey(manifestChunk.key, fragmentId);

        const fragmentKey = _getFragmentKey.key,
              parsedFragmentId = _getFragmentKey.fragmentId;
...
let MeshLayer = class MeshLayer extends withSegmentationLayerBackendState(withSharedVisibility(withChunkManager(PerspectiveViewRenderLayerBackend))) {
    constructor(rpc, options) {
        super(rpc, options);
...
export class MultiscaleFragmentChunk extends Chunk {
    ...
    serialize(msg, transfers) {
        super.serialize(msg, transfers);
        serializeMeshData(this.meshData, msg, transfers);
        const subChunkOffsets = this.subChunkOffsets;
...
let MultiscaleMeshLayer = class MultiscaleMeshLayer extends withSegmentationLayerBackendState(withSharedVisibility(withChunkManager(PerspectiveViewRenderLayerBackend))) {
    constructor(rpc, options) {
        super(rpc, options);
        this.source = this.registerDisposer(rpc.getRef(options['source']));
        this.registerDisposer(this.chunkManager.recomputeChunkPriorities.add(() => {
    ...
    updateChunkPriorities() {
        ...
        for (const _ref of this.attachments.values()) {
            ...
            mat4.multiply(modelViewProjectionMatrix, projectionParameters.viewProjectionMat, modelViewProjectionMatrix);
...
export function assignMeshFragmentData(chunk, data, vertexPositionFormat = VertexPositionFormat.float32) {
