...
describe('triangle_strips', () => {
    ...
    describe('getNextEdge', () => {
        it('works', () => {
            expect([[0, 0], [1, 0], [2, 0], [0, 1], [1, 1], [2, 1]].map(([edgeIndex, flipped]) => getNextEdge(edgeIndex + flipped * 4))).toEqual([2 + 0 * 4, 2 + 1 * 4, 1 + 1 * 4, 1 + 0 * 4, 0 + 0 * 4, 0 + 1 * 4]);
        });
    });
});
...
describe('triangle_strips', () => {
    ...
    it('works for random partitioned examples', () => {
        ...
        for (let iter = 0; iter < 10; ++iter) {
            const indices = makeRandomIndices(numTriangles, numVertices);
            const subChunkOffsets = Uint32Array.of(0, 20 * 3, (20 + 30) * 3, (20 + 30 + 40) * 3, (20 + 30 + 40 + 50) * 3);
            ...
            for (let subChunk = 0; subChunk < 4; ++subChunk) {
                ...
                const origTriangles = getTrianglesFromIndices(indices.subarray(subChunkOffsets[subChunk], subChunkOffsets[subChunk + 1]));
                expect(newTriangles).toEqual(origTriangles);
            }
        }
    });
