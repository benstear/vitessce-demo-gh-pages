...
var PngColorSpace;
(function (PngColorSpace) {
    PngColorSpace[PngColorSpace["GRAYSCALE"] = 0] = "GRAYSCALE";
    PngColorSpace[PngColorSpace["RGB"] = 2] = "RGB";
    PngColorSpace[PngColorSpace["PALETTE"] = 3] = "PALETTE";
    PngColorSpace[PngColorSpace["GRAYSCALE_ALPHA"] = 4] = "GRAYSCALE_ALPHA";
    PngColorSpace[PngColorSpace["RGBA"] = 6] = "RGBA";
...
function readHeader(buffer) {
    ...
    if (colorSpace === 0 /* GRAYSCALE */) {
            ...
            numChannels = 3;
        } else if (colorSpace === 3 /* PALETTE */) {
            dataWidth = 1;
            numChannels = 3;
        } else if (colorSpace === 6 /* RGBA */) {
            if (bitDepth !== 8 && bitDepth !== 16) {
                throw new Error(`png: invalid bit depth for RGBA colorspace. Got: ${bitDepth}`);
            }
            numChannels = 4;
        } else if (colorSpace === 4 /* GRAYSCALE_ALPHA */) {
            if (bitDepth !== 8 && bitDepth !== 16) {
...
export async function decompressPng(buffer, width, height, numComponents, bytesPerPixel, convertToGrayscale) {
    const m = await pngModulePromise;

    var _readHeader = readHeader(buffer);

    let sx = _readHeader.sx,
        sy = _readHeader.sy,
        dataWidth = _readHeader.dataWidth,
        numChannels = _readHeader.numChannels;
