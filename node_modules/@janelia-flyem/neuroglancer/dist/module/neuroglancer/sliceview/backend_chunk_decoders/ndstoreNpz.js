...
export async function decodeNdstoreNpzChunk(chunk, cancellationToken, response) {
    ...
    let shape = parseResult.shape;

    if (!arraysEqual(shape, chunkDataSize)) {
        throw new Error(`Shape ${_JSON$stringify(shape)} does not match chunkDataSize ${chunkDataSize.join()}`);
    }
