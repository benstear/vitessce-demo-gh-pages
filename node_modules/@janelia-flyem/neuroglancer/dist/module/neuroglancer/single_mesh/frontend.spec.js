...
describe('single_mesh/frontend', () => {
    it('attributes', () => {
        ...
        fragmentShaderTest({ vertexIndex: 'uint' }, {
            posX: 'float',
            posY: 'float',
            posZ: 'float',
            normX: 'float',
...
`);
            ...
            for (let index of [0, 1, 2, 32104, 100201, 143212]) {
                ...
                for (let i = 0; i < 3; ++i) {
                    expect(pos[i]).toEqual(vertexData.vertexPositions[index * 3 + i], `vertexPositions: index=${index}, i=${i}`);
                    expect(norm[i]).toEqual(vertexData.vertexNormals[index * 3 + i], `vertexNormals: index=${index}, i=${i}`);
                }
            }
        });
    });
