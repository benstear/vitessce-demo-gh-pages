...
describe('cancellation', () => {
    ...
    describe('makeCancellablePromise', () => {
        it('supports basic resolve behavior', done => {
            const promise = makeCancelablePromise(uncancelableToken, (resolve, _reject, _token) => {
                resolve(3);
            });
            promise.then(value => {
                expect(value).toBe(3);
        ...
        it('supports basic reject behavior', done => {
            const promise = makeCancelablePromise(uncancelableToken, (_resolve, reject, _token) => {
                reject(3);
            });
            promise.catch(value => {
                expect(value).toBe(3);
        ...
        it('unregisters the cancellation handler when the promise is fulfilled', () => {
            const source = new CancellationTokenSource();
            const log = [];
            makeCancelablePromise(source, (resolve, _reject, token) => {
                token.add(() => {
        ...
        it('unregisters the cancellation handler when the promise is rejected', () => {
            const source = new CancellationTokenSource();
            const log = [];
            makeCancelablePromise(source, (_resolve, reject, token) => {
                token.add(() => {
                    log.push('cancel called');
                });
                reject(1);
