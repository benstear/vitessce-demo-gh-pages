 ...
 */
import { getInsertPermutation, getMergeSplices, partitionArray, spliceArray, tile2dArray, transposeArray2d } from './array';
...
describe('spliceArray', () => {
    it('works for simple examaples', () => {
        const a = _Array$from(new Array(10), (_, i) => i);
        expect(spliceArray(a, [{ retainCount: 10, insertCount: 0, deleteCount: 0 }])).toEqual(a);
        expect(spliceArray(a, [{ retainCount: 5, deleteCount: 3, insertCount: 2 }])).toEqual([0, 1, 2, 3, 4, undefined, undefined, 8, 9]);
        expect(spliceArray(a, [{ retainCount: 2, deleteCount: 1, insertCount: 1 }, { retainCount: 3, deleteCount: 0, insertCount: 2 }])).toEqual([0, 1, undefined, 3, 4, 5, undefined, undefined, 6, 7, 8, 9]);
    });
});
describe('getMergeSplices', () => {
    it('works for simple examaples', () => {
        const compare = (a, b) => a - b;
        expect(getMergeSplices([0, 1, 2, 3], [0, 1, 2, 3], compare)).toEqual([{ retainCount: 4, deleteCount: 0, insertCount: 0 }]);
        expect(getMergeSplices([0, 1, 2, 3], [], compare)).toEqual([{ retainCount: 0, deleteCount: 4, insertCount: 0 }]);
        expect(getMergeSplices([], [0, 1, 2, 3], compare)).toEqual([{ retainCount: 0, deleteCount: 0, insertCount: 4 }]);
        expect(getMergeSplices([0, 1, 2, 3], [0, 1, 1.5, 2, 3], compare)).toEqual([{ retainCount: 2, deleteCount: 0, insertCount: 0 }, { retainCount: 0, deleteCount: 0, insertCount: 1 }, { retainCount: 2, deleteCount: 0, insertCount: 0 }]);
        expect(getMergeSplices([0, 1, 2, 3], [0, 1, 1.5, 3, 4], compare)).toEqual([{ retainCount: 2, deleteCount: 0, insertCount: 0 }, { retainCount: 0, deleteCount: 0, insertCount: 1 }, { retainCount: 0, deleteCount: 1, insertCount: 0 }, { retainCount: 1, deleteCount: 0, insertCount: 0 }, { retainCount: 0, deleteCount: 0, insertCount: 1 }]);
    });
});
