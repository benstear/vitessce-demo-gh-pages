 ...
 */
import { transformedBoundingBoxesEqual, CoordinateSpaceCombiner, coordinateSpacesEqual, emptyInvalidCoordinateSpace, homogeneousTransformSubmatrix, makeCoordinateSpace, makeIdentityTransform, newDimensionId, WatchableCoordinateSpaceTransform, coordinateTransformSpecificationFromJson } from './coordinate_transform';
    ...
    it('works for simple examples', () => {
        ...
        boxes.forEach((x, xIndex) => {
            boxes.forEach((y, yIndex) => {
                expect(transformedBoundingBoxesEqual(x, y)).toBe(x === y, `${xIndex}: ${_JSON$stringify(x)}, ` + `${yIndex}: ${_JSON$stringify(y)}`);
            });
        });
    });
});
describe('emptyCoordinateSpace', () => {
    it('has expected value', () => {
        ...
        expect(emptyInvalidCoordinateSpace.bounds).toEqual({ lowerBounds: new Float64Array(0), upperBounds: new Float64Array(0) });
    });
});
describe('CoordinateSpaceCombiner', () => {
    it('supports retain', () => {
        ...
        expect(combiner.dimensionRefCounts).toEqual(new _Map([['a', 1], ['d', 1]]));
        ...
        expect(combiner.combined.value.valid).toBeTruthy();
        binding();
        expect(coordinateSpacesEqual(emptyInvalidCoordinateSpace, combiner.combined.value)).toBeTruthy(combiner.combined.value);
    });
    it('handle renames', () => {
        ...
        const combiner = new CoordinateSpaceCombiner(new WatchableValue(emptyInvalidCoordinateSpace), () => true);
        combiner.bind(a);
        combiner.bind(b);
        ...
        expect(combiner.dimensionRefCounts).toEqual(new _Map([['a', 2], ['b', 1], ['c', 1]]));
        expect(combiner.combined.value.valid).toBeTruthy();
        ...
        expect(combiner.combined.value.units).toEqual(['s', 's', 'm']);
        expect(combiner.dimensionRefCounts).toEqual(new _Map([['d', 2], ['b', 1], ['c', 1]]));
        expect(combiner.combined.value.valid).toBeTruthy();
    ...
    it('works for identity transform', () => {
        const inputSpace = makeCoordinateSpace({
            ...
            boundingBoxes: [{
                ...
                transform: Float64Array.from([6, 7, 8, 9, 10, 11, 12, 13, 14] //
                )
            }]
        });
        ...
        const watchableTransform = new WatchableCoordinateSpaceTransform(makeIdentityTransform(inputSpace));
        watchableTransform.outputSpace.value = outputSpace;
        expect(watchableTransform.value.outputSpace.boundingBoxes).toEqual([{
            box: { lowerBounds: Float64Array.of(1, 2), upperBounds: Float64Array.of(5, 10) },
            transform: Float64Array.from([6 * 2, 7 * 3, 8 * 5, 9 * 2, 10 * 3, 11 * 5, 12 * 2, 13 * 3, 14 * 5] //
            )
        }]);
    });
});
describe('homogeneousTransformSubmatrix', () => {
    it('works for rank 2', () => {
        const orig = Float32Array.from([1, 2, 0, 3, 4, 0, 5, 6, 1]);
        const permuted = homogeneousTransformSubmatrix(Float32Array, orig, 2, [0, 1], [0, 1]);
        expect(permuted).toEqual(orig);
    });
    it('works for rank 4', () => {
        const orig = Float32Array.from([1, 2, 3, 4, 0, 3, 4, 5, 6, 0, 7, 8, 9, 10, 0, 11, 12, 13, 14, 0, 15, 16, 17, 18, 1] //
        );
        {
            const permuted = homogeneousTransformSubmatrix(Float32Array, orig, 4, [0, 1, 2, 3], [0, 1, 2, 3]);
            expect(permuted).toEqual(orig);
        }
        {
            const permuted = homogeneousTransformSubmatrix(Float32Array, orig, 4, [0, 1, 2, 3], [3, 0, 1, 2]);
            const expected = Float32Array.from([11, 12, 13, 14, 0, 1, 2, 3, 4, 0, 3, 4, 5, 6, 0, 7, 8, 9, 10, 0, 15, 16, 17, 18, 1] //
            );
...
//# sourceMappingURL=coordinate_transform.spec.js.map