...
`];
  ...
  async encode(h) {
    ...
    let C = this.level;
    C <= 0 && (C = iA), C > DA && (C = DA);
    const L = await x, Y = L.compress(h, C), t = new Uint8Array(Y);
  ...
  async decode(h, C) {
    ...
    try {
      ...
      return L.free_result(), C !== void 0 ? (C.set(t), C) : t;
    } catch (Y) {
      throw new Error(L.getExceptionMessage(Y).toString());
    }
  }
}, m(M, "codecId", "zstd"), m(M, "DEFAULT_CLEVEL", iA), m(M, "MAX_CLEVEL", DA), M), MI = JI;
export {
  MI as default
};
