...
const tinycolor = /* @__PURE__ */ getDefaultExportFromCjs(tinycolorExports);
...
function treeToSelectedSetMap(currTree, selectedNamePaths) {
  let result = [];
  selectedNamePaths.forEach((setNamePath) => {
    const node = treeFindNodeByNamePath(currTree, setNamePath);
    if (node) {
...
function treeToObsIdsBySetNames(currTree, selectedNamePaths) {
  const indices = [];
  selectedNamePaths.forEach((setNamePath) => {
    const node = treeFindNodeByNamePath(currTree, setNamePath);
    if (node) {
...
function treeToObsIndicesBySetNames(currTree, selectedNamePaths, obsIndexMap) {
  const indices = [];
  selectedNamePaths.forEach((setNamePath) => {
    const node = treeFindNodeByNamePath(currTree, setNamePath);
    if (node) {
...
function stratifyArrays(sampleEdges, sampleIdToObsIdsMap, sampleSets, sampleSetSelection, obsIndex, mergedCellSets, cellSetSelection, arraysToStratify, featureAggregationStrategy) {
  ...
  cellSetKeys.forEach((cellSetKey) => {
    ...
    sampleSetKeys.forEach((sampleSetKey) => {
      ...
      const sampleIdsInSampleSet = (_a = sampleSetInfo && sampleSetKey ? sampleSetInfo == null ? void 0 : sampleSetInfo.find((n) => isEqual(n.path, sampleSetKey)) : null) == null ? void 0 : _a.ids;
      ...
      const obsIdsInCellSet = (_b = cellSetInfo && cellSetKey ? cellSetInfo == null ? void 0 : cellSetInfo.find((n) => isEqual(n.path, cellSetKey)) : null) == null ? void 0 : _b.ids;
  ...
  for (let i = 0; i < obsIndex.length; i += 1) {
    ...
    const sampleSet = sampleId ? (sampleIdToSetMap == null ? void 0 : sampleIdToSetMap.get(sampleId)) || null : null;
...
function stratifyExpressionData(sampleEdges, sampleSets, sampleSetSelection, expressionData, obsIndex, mergedCellSets, geneSelection, cellSetSelection, cellSetColor, featureValueTransform, featureValueTransformCoefficient) {
  ...
  if (mergedCellSets && cellSetSelection && geneSelection && geneSelection.length >= 1 && expressionData) {
    ...
    for (let i = 0; i < obsIndex.length; i += 1) {
      ...
      const sampleId = sampleEdges == null ? void 0 : sampleEdges.get(obsId);
      const sampleSet = sampleId && hasSampleSetSelection ? sampleIdToSetMap == null ? void 0 : sampleIdToSetMap.get(sampleId) : null;
      ...
      geneKeys.forEach((geneKey, geneI) => {
        ...
        (_c = (_b = (_a = result.get(cellSet)) == null ? void 0 : _a.get(sampleSet)) == null ? void 0 : _b.get(geneKey)) == null ? void 0 : _c.push(transformedValue);
      });
    }
  ...
  Array.from(stratifiedResult.entries()).forEach(([cellSetKey, firstLevelInternMap]) => {
    ...
    Array.from(firstLevelInternMap.entries()).forEach(([sampleSetKey, secondLevelInternMap]) => {
      ...
      if (featureAggregationStrategy === "first") {
        ...
        values = subarrays.reduce((acc, curr) => acc.map((val, idx) => val + curr[idx]));
        if (featureAggregationStrategy === "mean") {
