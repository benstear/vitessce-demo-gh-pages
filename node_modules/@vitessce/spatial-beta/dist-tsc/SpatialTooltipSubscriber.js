...
function TooltipChild(props) {
    const { x, y, parentUuid, sourceUuid, width, height, info, tooltipsVisible, tooltipCrosshairsVisible, featureType, featureLabelsMap, } = props;
    const visible = parentUuid === sourceUuid ? tooltipsVisible : tooltipCrosshairsVisible;
    return (visible ? (_jsx(Tooltip2D, { x: x, y: y, parentUuid: parentUuid, sourceUuid: sourceUuid, parentWidth: width, parentHeight: height, children: _jsx(TooltipContent, { info: info, featureType: featureType, featureLabelsMap: featureLabelsMap }) })) : null);
}
...
export default function SpatialTooltipSubscriber(props) {
    const { parentUuid, width, height, hoverCoord, featureType, featureLabelsMap, 
    // Points
    obsPoints, pointLayerScopes, pointLayerCoordination, 
    // Spots
    obsSpots, spotLayerScopes, spotLayerCoordination, 
    // Segmentations
    obsSegmentationsLocations, segmentationLayerScopes, segmentationChannelScopesByLayer, segmentationChannelCoordination, obsSegmentationsSetsData, 
    // Images
    ...
    return (_jsxs(_Fragment, { children: [projectedHoverCoord && imageLayerScopes?.map((layerScope) => {
                ...
                return (tooltipsVisible && pixelHighlight && spatialLayerVisible ? (_jsx(TooltipChild, { parentUuid: parentUuid, sourceUuid: sourceUuid, width: width, height: height, info: {
                    ...
                    }, x: projectedHoverCoord?.[0], y: projectedHoverCoord?.[1] + (yOffset += 30), featureType: featureType, featureLabelsMap: featureLabelsMap }, layerScope)) : null);
                ...
                if (!xy)
                    return null;
                const [x, y] = xy;
                return (_jsx(TooltipChild, { parentUuid: parentUuid, sourceUuid: sourceUuid, tooltipsVisible: tooltipsVisible, tooltipCrosshairsVisible: tooltipCrosshairsVisible, width: width, height: height, info: obsInfo, x: x, y: y + (yOffset += (20 + 15 * Object.keys(obsInfo).length)), featureType: featureType, featureLabelsMap: featureLabelsMap }, `${layerScope}-${channelScope}`));
                ...
                return (_jsx(TooltipChild, { parentUuid: parentUuid, sourceUuid: sourceUuid, tooltipsVisible: tooltipsVisible, tooltipCrosshairsVisible: tooltipCrosshairsVisible, width: width, height: height, info: {
                    ...
                    }, x: x, y: y + (yOffset += 30), featureType: featureType, featureLabelsMap: featureLabelsMap }, layerScope));
