...
import { Grid, Typography, Slider, } from '@vitessce/styles';
...
function DimensionSlider(props) {
    ...
    return (_jsxs(Grid, { container: true, direction: "row", justifyContent: "flex-start", alignItems: "center", children: [_jsx(Grid, { size: 1, children: _jsxs(Typography, { style: { marginBottom: 0 }, children: [label, ":"] }) }), _jsx(Grid, { size: 11, children: _jsx(Slider, { value: val, onChange: (e, v) => setSliceValue(v), valueLabelDisplay: "auto", valueLabelFormat: v => abbreviateNumber(v), getAriaLabel: index => `${label}-axis clipping plane slider ${index === 0 ? 'min' : 'max'}`, min: min, max: max, step: 0.005, orientation: "horizontal" }) })] }, label));
}
