...
export default function HelpTooltip(props) {
    const { title, content, overlayClassName = 'helpTooltip', placement = 'top', trigger = 'hover', mouseEnterDelay = 0.2, mouseLeaveDelay = 0, destroyTooltipOnHide = true, } = props;
    const spanRef = useRef();
    const getTooltipContainer = useVitessceContainer(spanRef);
    const overlay = title || content;
    const { classes } = useHelpTooltipStyles();
    return (_jsxs(_Fragment, { children: [overlayClassName === 'helpTooltip' ? (_jsx(HelpTooltipGlobalStyles, { classes: classes })) : (_jsx(PopoverGlobalStyles, { classes: classes })), _jsx("span", { ref: spanRef }), _jsx(RcTooltip, { ...props, placement: placement, trigger: trigger, mouseEnterDelay: mouseEnterDelay, mouseLeaveDelay: mouseLeaveDelay, destroyOnHidden: destroyTooltipOnHide, getTooltipContainer: getTooltipContainer, classNames: { root: classes[overlayClassName] }, overlay: overlay })] }));
}
