...
function NamedSetNodeStatic(props) {
    ...
    return (_jsxs("span", { children: [_jsx(HelpTooltip, { title: tooltipText, ...tooltipProps, children: _jsx("button", { type: "button", onClick: onClick, onKeyPress: e => callbackOnKeyPress(e, 'v', () => onNodeView(path)), className: classes.titleButton, children: title }) }), popoverMenuConfig.length > 0 ? (_jsx(PopoverMenu, { menuConfig: makeNodeViewMenuConfig(props), color: level > 0 && editable ? (color || getDefaultColor(theme)) : null, setColor: c => onNodeSetColor(path, c), children: _jsx("span", { children: _jsx(MenuSVG, { className: classes.nodeMenuIcon, "aria-label": "Open Node View Menu" }) }) })) : null, level > 0 && isChecking ? checkbox : null, level > 0 && (_jsx("span", { className: classes.nodeSizeLabel, children: niceSize }))] }));
}
/**
...
export default class TreeNode extends RcTreeNode {
    ...
    renderSelector = () => {
        ...
        const { prefixCls: prefixClass, draggable, } = this.context;
        const onDragStart = (e) => {
            onDragStartProp();
            this.onDragStart(e);
        };
    ...
    renderSwitcher = () => {
        ...
        const { prefixCls: prefixClass, onNodeExpand, } = this.context;
    ...
    render() {
        ...
        const { prefixCls: prefixClass, filterTreeNode, draggable, } = this.context;
