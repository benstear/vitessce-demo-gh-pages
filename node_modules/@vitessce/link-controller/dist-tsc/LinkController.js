...
import { useViewConfigStoreApi, createLoaders, } from '@vitessce/vit-s';
import { Checkbox, Grid, } from '@vitessce/styles';
import { log } from '@vitessce/globals';
export default function LinkController(props) {
    const { linkIDInit, authToken, linkEndpoint, websocketEndpoint, fileTypes, coordinationTypes, stores, } = props;
    ...
    const setConfig = useCallback((newConfig) => {
        viewConfigStoreApi.setState({
            ...
            loaders: createLoaders(newConfig.datasets, newConfig.description, fileTypes, coordinationTypes, stores),
        });
    }, [viewConfigStoreApi]);
    ...
    useEffect(() => viewConfigStoreApi.subscribe(state => ({
    ...
    }, { equalityFn: shallow }), [viewConfigStoreApi, onConfigChange]);
    useEffect(() => {
        if (linkID !== null) {
    ...
    return (_jsx(_Fragment, { children: _jsxs("span", { children: [_jsxs("p", { style: { textAlign: 'justify' }, children: ["To join the same session navigate to ", _jsx("a", { href: "https://vitessce.link", children: "https://vitessce.link" }), " and enter the ", _jsx("b", { children: "Link ID" }), " displayed here in the view. The session is synced as long as the ", _jsx("b", { children: "Link Active" }), " Checkbox is activated."] }), _jsxs(Grid, { container: true, direction: "row", sx: { gridGap: '10px' }, children: [_jsx(Grid, { size: 5, sx: { whiteSpace: 'nowrap' }, children: _jsxs("p", { style: { fontSize: '25px' }, children: ["Link ID:\u00A0\u00A0", _jsx("b", { children: linkID })] }) }), _jsxs(Grid, { size: 5, sx: { fontSize: '25px', whiteSpace: 'nowrap' }, children: ["Link Active: ", _jsx(Checkbox, { sx: { marginTop: '-2px', marginLeft: '-10px' }, color: "primary", checked: sync, onChange: e => setSync(e.target.checked) })] })] })] }) }));
}
