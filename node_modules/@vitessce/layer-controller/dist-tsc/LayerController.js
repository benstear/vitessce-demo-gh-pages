...
import { GLOBAL_LABELS, getSourceFromLoader, getMultiSelectionStats, DOMAINS, canLoadResolution, } from '@vitessce/spatial-utils';
import { Grid, Button, Slider, Tabs, Tab, InputLabel, Accordion, AccordionDetails, AccordionSummary, Box, Add as AddIcon, ExpandMore as ExpandMoreIcon, Visibility as VisibilityIcon, VisibilityOff as VisibilityOffIcon, } from '@vitessce/styles';
...
export default function LayerController(props) {
    ...
    const FullController = (_jsxs(_Fragment, { children: [_jsx(LayerOptions, { channels: channels, opacity: opacity, colormap: colormap, transparentColor: transparentColor, domainType: domainType, 
                ...
                globalControlLabels: labels.filter(label => GLOBAL_LABELS.includes(label)), globalLabelValues: globalLabelValues, handleOpacityChange: setOpacity, handleColormapChange: setColormap, handleGlobalChannelsSelectionChange: handleGlobalChannelsSelectionChange, handleTransparentColorChange: setTransparentColor, disableChannelsIfRgbDetected: photometricInterpretation === 'RGB', handleDomainChange: handleDomainChange, shouldShowTransparentColor: shouldShowTransparentColor, shouldShowDomain: shouldShowDomain, shouldShowColormap: shouldShowColormap, use3d: use3d, loader: loader, handleMultiPropertyChange: handleMultiPropertyChange, resolution: resolution, disable3d: disable3d, setRasterLayerCallback: setImageLayerCallback, setAreAllChannelsLoading: setAreAllChannelsLoading, setViewState: setViewState, spatialHeight: spatialHeight, spatialWidth: spatialWidth, modelMatrix: modelMatrix }), photometricInterpretation === 'RGB'
                ? null
                : channelControllers, photometricInterpretation === 'RGB' ? null : (_jsx(Button, { disabled: channels.length === viv.MAX_CHANNELS, onClick: handleChannelAdd, fullWidth: true, variant: "outlined", style: buttonStyles, startIcon: _jsx(AddIcon, {}), size: "small", children: "Add Channel" }))] }));
    return (_jsxs(Accordion, { slotProps: {
            root: { elevation: 4, className: controllerSectionClasses.accordionRoot },
        }, onChange: (e, expanded) => !disabled
            && setIsExpanded(expanded && e?.target?.attributes?.role?.value === 'presentation'), expanded: !disabled && isExpanded, id: `layer-controls-accordion-${layerControlsId}`, children: [_jsx(AccordionSummary, { slotProps: {
                    content: { className: accordionClasses.content },
                }, expandIcon: _jsx(ExpandMoreIcon, { role: "presentation" }), "aria-controls": `layer-${name}-controls`, "aria-expanded": isExpanded, children: _jsxs(Grid, { container: true, direction: "column", justifyContent: "center", sx: { flexGrow: 1 }, children: [_jsxs(Grid, { container: true, size: 12, direction: "row", alignItems: "flex-start", classes: { item: overflowEllipsisGridClasses.item }, children: [_jsx(Box, { component: "div", role: "button", "aria-label": "Toggle layer visibility", onClick: (e) => {
                                        if (!disabled) {
                                    ...
                                    }, className: accordionClasses.accordionVisibilityIconBox, children: _jsx(Visibility, {}) }), _jsx(Box, { component: "div", className: accordionClasses.accordionNameBox, children: name })] }), !disabled && !isExpanded && !use3d && (_jsxs(Grid, { container: true, direction: "row", sx: { justifyContent: 'flex-start', flexGrow: 1 }, children: [_jsx(Grid, { size: 5, children: _jsx(InputLabel, { htmlFor: `layer-${name}-opacity-closed`, classes: { root: inputLabelClasses.inputLabelRoot }, children: "Opacity:" }) }), _jsx(Grid, { size: 6, children: _jsx(Slider, { id: `layer-${name}-opacity-closed`, value: opacity, onChange: (e, v) => setOpacity(v), valueLabelDisplay: "auto", "aria-label": `Adjust opacity for layer ${name}`, min: 0, max: 1, step: 0.01, orientation: "horizontal" }) })] }))] }) }), _jsxs(AccordionDetails, { id: `layer-${name}-controls`, children: [useVolumeTabs ? (_jsxs(_Fragment, { children: [_jsxs(Tabs, { value: tab, onChange: handleTabChange, "aria-label": "Change the layer tab type", style: { height: '24px', minHeight: '24px' }, children: [_jsx(Tab, { label: "Channels", "aria-label": "Channels tab", style: {
                                            fontSize: '.75rem',
                                            bottom: 12,
                                            width: '50%',
                                            minWidth: '50%',
