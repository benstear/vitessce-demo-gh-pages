...
import { Grid, Typography, Button, makeStyles, FormControl, NativeSelect, InputLabel, Slider, } from '@vitessce/styles';
...
const Slicer = ({ xSlice, ySlice, zSlice, handleSlicerSetting, loader, use3d, }) => {
    ...
    const Slicers = sliceValuesAndSetSliceFunctions.map(([val, setVal, label, [min, max]]) => (_jsxs(Grid, { container: true, direction: "row", justifyContent: "flex-start", alignItems: "center", children: [_jsx(Grid, { size: 1, children: _jsxs(Typography, { className: !use3d ? classes.disabled : classes.enabled, style: { marginBottom: 0 }, children: [label, ":"] }) }), _jsx(Grid, { size: 11, children: _jsx(Slider, { disabled: !use3d, className: !use3d ? classes.disabled : classes.enabled, value: val, onChange: (e, v) => setVal(v), valueLabelDisplay: "auto", valueLabelFormat: v => abbreviateNumber(v), getAriaLabel: index => `Clipping plane ${label} slider ${index === 0 ? 'min' : 'max'}`, min: min, max: max, step: 0.005, orientation: "horizontal" }) })] }, label)));
...
function RenderingModeSelect({ handleRenderingModeChange, renderingMode, use3d, }) {
    ...
    return (_jsxs(FormControl, { fullWidth: true, children: [_jsx(InputLabel, { htmlFor: "rendering-mode-select", variant: "standard", children: "Rendering Mode" }), _jsx(NativeSelect, { onChange: e => handleRenderingModeChange(e.target.value), value: use3d ? renderingMode : '', inputProps: {
                    name: 'rendering-mode',
...
const ReCenterButton = ({ setViewState, use3d, spatialHeight, spatialWidth, loader, modelMatrix, }) => (_jsx(Grid, { size: "auto", children: _jsx(Button, { onClick: () => {
