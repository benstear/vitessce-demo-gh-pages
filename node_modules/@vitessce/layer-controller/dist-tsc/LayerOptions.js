...
function VolumeDropdown({ loader: loaderWithMeta, handleMultiPropertyChange, resolution: currResolution, disable3d, setRasterLayerCallback, setAreAllChannelsLoading, setViewState, spatialHeight, spatialWidth, channels, use3d, modelMatrix, }) {
    ...
    return (_jsx(_Fragment, { children: _jsxs(NativeSelect, { value: currResolution, onChange: e => handleChange(e.target.value === '2D' ? e.target.value : Number(e.target.value)), inputProps: { 'aria-label': 'Resolution selector' }, classes: { root: classes.selectRoot }, children: [_jsx("option", { value: "2D", children: "2D Visualization" }, "2D"), Array.from({ length: loader.length })
                    .fill(0)
...
function ColormapSelect({ value, inputId, handleChange }) {
    const { classes } = useSelectStyles();
    return (_jsxs(NativeSelect, { onChange: e => handleChange(e.target.value === '' ? null : e.target.value), value: value, inputProps: { name: 'colormap', id: inputId, 'aria-label': 'Colormap selector' }, style: { width: '100%' }, classes: { root: classes.selectRoot }, children: [_jsx("option", { "aria-label": "None", value: "", children: "None" }), COLORMAP_OPTIONS.map(name => (_jsx("option", { value: name, children: name }, name)))] }));
}
function TransparentColorCheckbox({ value, inputId, handleChange }) {
    return (_jsx(Checkbox, { style: { float: 'left', padding: 0 }, color: "default", onChange: () => {
        ...
        }, checked: Boolean(value), id: inputId, slotProps: { input: { 'aria-label': 'Enable or disable color transparency' } } }));
}
/**
...
function OpacitySlider({ value, inputId, handleChange }) {
    ...
    return (_jsx(Slider, { slotProps: { valueLabel: { className: classes.sliderValueLabel } }, value: value, onChange: (e, v) => handleChange(v), valueLabelDisplay: "auto", "aria-label": "Layer opacity slider", id: inputId, min: 0, max: 1, step: 0.01, orientation: "horizontal" }));
}
/**
...
function SliderDomainSelector({ value, inputId, handleChange }) {
    const { classes } = useSelectStyles();
    return (_jsx(NativeSelect, { onChange: e => handleChange(e.target.value), value: value, id: inputId, inputProps: { name: 'domain-selector', 'aria-label': 'Domain type selector' }, style: { width: '100%' }, classes: { root: classes.selectRoot }, children: DOMAIN_OPTIONS.map(name => (_jsx("option", { value: name, children: name }, name))) }));
}
/**
...
function LayerOption({ name, inputId, children }) {
    return (_jsxs(Grid, { container: true, direction: "row", alignItems: "center", justifyContent: "center", children: [_jsx(Grid, { size: 6, children: _jsxs(InputLabel, { htmlFor: inputId, children: [name, ":"] }) }), _jsx(Grid, { size: 6, children: children })] }));
}
/**
...
function LayerOptions({ colormap, opacity, handleColormapChange, handleOpacityChange, handleTransparentColorChange, globalControlLabels, globalLabelValues, handleGlobalChannelsSelectionChange, handleSliderChange, handleDomainChange, transparentColor, channels, domainType, disableChannelsIfRgbDetected, shouldShowTransparentColor, shouldShowDomain, shouldShowColormap, use3d, loader, handleMultiPropertyChange, resolution, disable3d, setRasterLayerCallback, setAreAllChannelsLoading, setViewState, spatialHeight, spatialWidth, modelMatrix, }) {
    ...
    return (_jsxs(Grid, { container: true, direction: "column", sx: { width: '100%' }, children: [hasZStack
                && !disable3d
