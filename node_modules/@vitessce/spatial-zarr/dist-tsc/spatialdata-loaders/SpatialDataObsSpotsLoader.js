...
import { normalizeCoordinateTransformations, coordinateTransformationsToMatrix, } from '@vitessce/spatial-utils';
...
export default class SpatialDataObsSpotsLoader extends AbstractTwoStepLoader {
    async loadModelMatrix() {
        ...
        const { 'encoding-type': encodingType, spatialdata_attrs: { geos = {}, version: attrsVersion, }, } = zattrs;
        const hasExpectedAttrs = (encodingType === 'ngff:shapes'
            && ((geos?.name === 'POINT'
                && geos?.type === 0
                && attrsVersion === '0.1') || attrsVersion === '0.2'));
        if (!hasExpectedAttrs) {
            throw new AbstractLoaderError('Unexpected values for encoding-type or spatialdata_attrs for SpatialData shapes');
        }
        ...
        const coordinateTransformationsFromFile = (zattrs?.coordinateTransformations || DEFAULT_COORDINATE_TRANSFORMATIONS).filter(({ input: { name: inputName }, output: { name: outputName } }) => (inputName === 'xy' && outputName === coordinateSystem));
